    ...
    if host:    
        ...
        if switchmodel == "1":
            ...
            if "Username" in model_check.decode('ascii'):
                tn.close()
                story = "Выбрана неверная модель коммутатора!, Выберите Edge-core!"
                return story
            ...
            if "User name" in model_check.decode('ascii'):
                tn.close()
                story = "Выбрана неверная модель коммутатора! Выберите ZyXEL!"
                return story
...
def mactest(host, iface, details, switchmodel):
...
def errortest(host, iface, details, switchmodel):
...
def iptest(host, iface, details, switchmodel, noun):
...
def looptest(host, iface, details, switchmodel):
...
def portactivate(host, iface, details, switchmodel):
...
def sysinfo(host, iface, details, switchmodel):
...
def showlog(host, iface, details, switchmodel):
...
def showprofile(host, iface, details, switchmodel):
